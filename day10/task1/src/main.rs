...
fn solve(field: &str) -> i32 {
    ...
    q.push_back((start_row, start_col, cnt));

    while !q.is_empty() {
        let (row, col, n) = q.pop_front().unwrap();
        ...
        if n > cnt {
            cnt = n;
            // println!("found new length at {}, {} = {}", row, col, n);
        } else {
            // println!("encountered {}, {} = {}", row, col, n);
        }
