...
fn solve(field: &str) -> i32 {
    ...
    for (row, line) in field.lines().enumerate() {
    ...
    while !q.is_empty() {
        ...
        if visited.contains(&(row, col)) {
            // println!("already been at {}, {} = {}", row, col, n);
            continue;
        }

        visited.insert((row, col));

        let cur = char_at_field(field, row, col);

        if row < rows - 1 && (cur == 'S' || cur == '|' || cur == '7' || cur == 'F') {
            let ch = char_at_field(field, row + 1, col);
        ...
        if row > 0 && (cur == 'S' || cur == '|' || cur == 'J' || cur == 'L') {
            let ch = char_at_field(field, row - 1, col);
        ...
        if col < cols - 1 && (cur == 'S' || cur == '-' || cur == 'L' || cur == 'F') {
            let ch = char_at_field(field, row, col + 1);
        ...
        if col > 0 && (cur == 'S' || cur == '-' || cur == '7' || cur == 'J') {
            let ch = char_at_field(field, row, col - 1);
    ...
    let mut islands: HashSet<(usize, usize)> = HashSet::new();

    for row in 0..rows {
        for col in 0..cols {
            if path.contains(&(row, col)) || islands.contains(&(row, col)) {
                continue;
            }

            let block = bfs(rows, cols, &path, row, col);

            islands.extend(&block);
        }
    }
    ...
    for row in 0..rows {
        ...
        // let mut tmp_inside = HashSet::new();

        for col in 0..cols {
            let ch = char_at_field(field, row, col);

            if path.contains(&(row, col)) && (ch == 'S' || ch == '|' || ch == 'L' || ch == 'J') {
                is_inside = !is_inside;
            } else if islands.contains(&(row, col)) && is_inside {
...
fn bfs(rows: usize, cols: usize, path: &HashSet<(usize, usize)>, start_row: usize, start_col: usize) -> HashSet<(usize, usize)> {
    let mut q = VecDeque::new();
    let mut visited = HashSet::new();
    ...
    while !q.is_empty() {
        ...
        if visited.contains(&(row, col)) || path.contains(&(row, col)) {
            continue;
        }
...
fn char_at_field(field: &str, row: usize, col: usize) -> char {
    field
        .lines()
        .nth(row)
        .unwrap()
