...
"#.trim();
    ...
    let cols = s.lines().nth(0).map(|l| l.len()).unwrap_or(0);
    let rows = s.lines().count();

    let mut sum = 0;

    for row in 0..rows {
        let line = s.lines().nth(row).unwrap();
        ...
        for col in 0..cols {
            ...
            if ch.is_ascii_digit() {
                ...
                if n > 0 {
                    // if first_col.unwrap() > 0 && line.chars().nth(first_col.unwrap() - 1).unwrap() == '-' {
                    //     n = n * -1;
                    //     first_col = first_col.map(|x| x - 1);
                    // }

                    println!("found number: {} at {}, {}..{}", n, row, first_col.unwrap(), col);

                    let mut fl = false;

                    if !fl && col > 0 && first_col.unwrap() > 0 {
                        let col1 = first_col.unwrap() - 1;
                        let ch0 = line.chars().nth(col1).unwrap();

                        println!("> check1: {} at {}, {}", ch0, row, col1);

                        if ch0 != '.' && !ch0.is_ascii_digit() {
                            // success
                            println!("> adj1: {} at {}, {}..{} :: {} at {}, {}", n, row, first_col.unwrap(), col, ch0, row, col1);
                            fl = true;
                        }
                    }
                    ...
                    if !fl && col < cols - 1 {
                        let col2 = col;
                        let ch1 = line.chars().nth(col2).unwrap();

                        println!("> check2: {} at {}, {}", ch1, row, col2);

                        if ch1 != '.' && !ch1.is_ascii_digit() {
                            // success
                            println!("> adj2: {} at {}, {}..{} :: {} at {}, {}", n, row, first_col.unwrap(), col, ch1, row, col2);
                            fl = true;
                        }
                    }

                    if !fl && row > 0 {
                        let line1 = s.lines().nth(row - 1).unwrap();

                        if first_col.unwrap() > 0 {
                            for col3 in (first_col.unwrap() - 1)..=cmp::min(cols - 1, col) {
                                let ch2 = line1.chars().nth(col3).unwrap();

                                println!("> check3: {} at {}, {}", ch2, row - 1, col3);

                                if ch2 != '.' && !ch2.is_ascii_digit() {
                                    // success
                                    println!("> adj3: {} at {}, {}..{} :: {} at {}, {}", n, row, first_col.unwrap(), col, ch2, row - 1, col3);
                                    fl = true;
                                    break;
                                }
                            }
                        } else {
                            for col3 in first_col.unwrap()..=cmp::min(cols - 1, col) {
                                let ch2 = line1.chars().nth(col3).unwrap();

                                println!("> check4: {} at {}, {}", ch2, row - 1, col3);

                                if ch2 != '.' && !ch2.is_ascii_digit() {
                                    // success
                                    println!("> adj4: {} at {}, {}..{} :: {} at {}, {}", n, row, first_col.unwrap(), col, ch2, row - 1, col3);
                                    fl = true;
                                    break;
                                }
                            }
                    ...
                    if !fl && row < rows - 1 {
                        let line2 = s.lines().nth(row + 1).unwrap();

                        if first_col.unwrap() > 0 {
                            for col4 in (first_col.unwrap() - 1)..=cmp::min(cols - 1, col) {
                                let ch3 = line2.chars().nth(col4).unwrap();

                                println!("> check5: {} at {}, {}", ch3, row + 1, col4);

                                if ch3 != '.' && !ch3.is_ascii_digit() {
                                    // success
                                    println!("> adj5: {} at {}, {}..{} :: {} at {}, {}", n, row, first_col.unwrap(), col, ch3, row + 1, col4);
                                    fl = true;
                                    break;
                                }
                            }
                        } else {
                            for col4 in first_col.unwrap()..=cmp::min(cols - 1, col) {
                                let ch3 = line2.chars().nth(col4).unwrap();

                                println!("> check6: {} at {}, {}", ch3, row + 1, col4);

                                if ch3 != '.' && !ch3.is_ascii_digit() {
                                    // success
                                    println!("> adj6: {} at {}, {}..{} :: {} at {}, {}", n, row, first_col.unwrap(), col, ch3, row + 1, col4);
                                    fl = true;
                                    break;
                                }
                            }
        ...
        if n > 0 {
            // println!("found last number: {}", n);

            let mut fl = false;
            let col = cols - 1;

            if !fl && col > 0 {
                let col1 = first_col.unwrap_or(0) - 1;
                let ch0 = line.chars().nth(col1).unwrap();

                println!("> check7: {} at {}, {}", ch0, row, col1);

                if ch0 != '.' && !ch0.is_ascii_digit() {
                    // success
                    println!("> adj7: {} at {}, {}..{} :: {} at {}, {}", n, row, first_col.unwrap(), col, ch0, row, col1);
                    fl = true;
                }
            }
            ...
            if !fl && col < cols - 1 {
                ...
                println!("> check8: {} at {}, {}", ch1, row, col2);

                if ch1 != '.' && !ch1.is_ascii_digit() {
                    // success
                    println!("> adj8: {} at {}, {}..{} :: {} at {}, {}", n, row, first_col.unwrap(), col, ch1, row, col2);
                    fl = true;
                }
            }
            ...
            if !fl && row > 0 {
                ...
                for col3 in cmp::max(0, first_col.unwrap_or(0) - 1)..=cmp::min(cols - 1, col) {
                    let ch2 = line1.chars().nth(col3).unwrap();

                    println!("> check9: {} at {}, {}", ch2, row - 1, col3);

                    if ch2 != '.' && !ch2.is_ascii_digit() {
                        // success
                        println!("> adj9: {} at {}, {}..{} :: {} at {}, {}", n, row, first_col.unwrap(), col, ch2, row - 1, col3);
                        fl = true;
                        break;
                    }
                }
            ...
            if !fl && row < rows - 1 {
                ...
                for col4 in (first_col.unwrap() - 1)..=cmp::min(cols - 1, col) {
                    ...
                    println!("> check10: {} at {}, {}", ch3, row + 1, col4);

                    if ch3 != '.' && !ch3.is_ascii_digit() {
                        // success
                        println!("> adj10: {} at {}, {}..{} :: {} at {}, {}", n, row, first_col.unwrap(), col, ch3, row + 1, col4);
                        fl = true;
                        break;
                    }
                }
