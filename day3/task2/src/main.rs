fn find_nums(s: &str, row: usize, col: usize, rows: usize, cols: usize) -> Vec<u32> {
    let mut nums = Vec::new();

    let col0 = if col > 0 { col - 1 } else { col };

    let col1 = if col < cols { col + 1 } else { col };

    let row0 = if row > 0 { row - 1 } else { row };

    let row1 = if row < rows { row + 1 } else { row };

    let mut r = row0;

    while r <= row1 {
        let line = s.lines().nth(r).unwrap();
        ...
        while c <= col1 {
            ...
            if ch.is_ascii_digit() {
                ...
                while c0 > 0 && line.chars().nth(c0).unwrap().is_ascii_digit() {
                    c0 -= 1;
                }

                while c1 < cols && line.chars().nth(c1).unwrap().is_ascii_digit() {
                    c1 += 1;
                }

                while !line.chars().nth(c0).unwrap().is_ascii_digit() {
                    c0 += 1;
                }
                ...
                for i in c0..=c1 {
                    let ch_d = line.chars().nth(i).unwrap();
                    let d = ch_d.to_digit(10).unwrap();
                    n = (n * 10) + d;
                }
...
"#.trim();
    ...
    for row in 0..rows {
        ...
        for col in 0..cols {
            ...
            if ch == '*' {
                let nums = find_nums(s, row, col, rows, cols);

                if nums.len() == 2 {
                    // println!("found gear at {}, {} and ratios {}, {}", row, col, nums[0], nums[1]);
                    sum += nums[0] * nums[1];
                }
            }
        }
