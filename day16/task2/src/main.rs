...
fn solve(field: &str) -> usize {
    ...
    {
        let var1 = (0, 0, Dir::Right); variants.push(var1);
        let var2 = (0, cols - 1, Dir::Left); variants.push(var2);
        let var3 = (rows - 1, 0, Dir::Right); variants.push(var3);
        let var4 = (rows - 1, cols - 1, Dir::Right); variants.push(var4);
    }

    for row in 0..rows {
        let var1 = (row, 0usize, Dir::Right); variants.push(var1);
        let var2 = (row, cols - 1, Dir::Left); variants.push(var2);
    }

    for col in 0..cols {
        let var1 = (0usize, col, Dir::Down); variants.push(var1);
        let var2 = (rows - 1, col, Dir::Up); variants.push(var2);
    }

    variants
        .iter()
        .map(|(row, col, dir)| par_solve(&lines, rows, cols, *row, *col, &dir))
        .max()
        .unwrap_or(0)
}
