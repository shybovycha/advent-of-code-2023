...
fn is_game_valid(game_str: &str) -> bool {
    let pileRe = Regex::new(r"(\d+)\s+(red|green|blue)").unwrap();

    // let mut cubes = HashMap::new();

    for pile in game_str.split(",") {
        let pileCaps = pileRe.captures(pile).unwrap();

        let cubesCap = pileCaps.get(1).unwrap().as_str();
        let color = pileCaps.get(2).unwrap().as_str();

        let cubes = cubesCap.parse::<u32>().unwrap();

        if (color == "red" && cubes > 12) || (color == "green" && cubes > 13) || (color == "blue" && cubes > 14) {
            return false
        }

        // cubes.insert(color, cubes);
    }
...
Game 100: 8 green, 7 blue, 1 red; 10 blue, 2 green, 5 red; 12 blue, 1 green, 1 red; 9 green, 9 blue, 2 red; 1 blue, 5 red, 3 green
    ...
    for line in s.trim().lines() {
        let gameCaptures = gameRe.captures(line).unwrap();

        let idStr = gameCaptures.get(1).unwrap().as_str();
        let id = idStr.parse::<u32>().unwrap();

        let mut games = gameCaptures.get(2).unwrap().as_str().split(";"); //.collect::<Vec<&str>>(); // .join(",");

        if games.all(|g| is_game_valid(g)) {
            sum += id
        } else {
            println!("game {id} is invalid {}", line);
        }
    }
