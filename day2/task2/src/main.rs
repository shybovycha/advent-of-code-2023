...
fn min_cubes_in(pulls: Vec<&str>) -> (u32, u32, u32) {
    let pile_re = Regex::new(r"(\d+)\s+(red|green|blue)").unwrap();

    let mut cache: HashMap<String, u32> = HashMap::new();

    for pull in pulls {
        for pile in pull.split(",") {
            let pile_caps = pile_re.captures(pile).unwrap();

            let cubes_cap = pile_caps.get(1).unwrap().as_str();
            let color = pile_caps.get(2).unwrap().as_str();

            let cubes = cubes_cap.parse::<u32>().unwrap();

            if !cache.contains_key(color) || cache.get(color).unwrap() < &cubes {
                cache.insert(color.to_string(), cubes);
            }
        }
    }

    let min_red = cache.get("red").unwrap_or(&0);
    let min_green = cache.get("green").unwrap_or(&0);
    let min_blue = cache.get("blue").unwrap_or(&0);
...
Game 100: 8 green, 7 blue, 1 red; 10 blue, 2 green, 5 red; 12 blue, 1 green, 1 red; 9 green, 9 blue, 2 red; 1 blue, 5 red, 3 green
    ...
    for line in s.trim().lines() {
        ...
        let pulls = game_captures.get(2).unwrap().as_str().split(";");

        let (min_red, min_green, min_blue) = min_cubes_in(pulls.collect());

        let power = min_red * min_green * min_blue;

        // println!("game: \"{}\"\n>> ({}, {}, {}), power: {}", line, min_red, min_green, min_blue, power);

        sum += Into::<u64>::into(power);
    }
