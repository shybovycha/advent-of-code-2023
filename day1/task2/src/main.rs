...
fn main() {
    ...
    // let numbers: Vec<u32> = s.trim().lines().map(parse_line_as_number).collect();
    for line in s.trim().lines() {
        // let n1 = parse_line_as_number(line);
        let n2 = moo(line);

        // if n1 != n2 {
        //     println!("wrong: {}, n1 = {}, n2 = {}", line, n1, n2);
        // }
    ...
    // println!("sum = {} or {}", answer1, answer2)
    println!("sum = {}", answer2)
}

fn moo(line_raw: &str) -> u32 {
    let regexes = vec![
        ("one", "1"),
        ("two", "2"),
        ("three", "3"),
    ...
    let replacements: Vec<(usize, &str, &str)> = regexes
      .iter()
      .flat_map(|&(re, n)| line_raw.match_indices(re).map(move |(p, re1)| (p, re1, n)))
      .collect();
    ...
    let line1 = replacements
      .iter()
      .min_by(|(p1, _, _), (p2, _, _)| p1.cmp(p2))
      .map(|(_, re1, n1)| line_raw.replace(re1, n1))
      .unwrap_or(line_raw.to_owned());

    let line2 = replacements
      .iter()
      .max_by(|(p1, _, _), (p2, _, _)| p1.cmp(p2))
      .map(|(_, re2, n2)| line_raw.replace(re2, n2))
      .unwrap_or(line_raw.to_owned());

    let digits1: Vec<u32> = line1.chars().filter_map(|ch| ch.to_digit(10)).collect();
    let digits2: Vec<u32> = line2.chars().filter_map(|ch| ch.to_digit(10)).collect();

    let first = digits1.first().unwrap_or(&0);
    let last = digits2.last().unwrap_or(&0);

    let num = (first * 10) + last;

    let rs = replacements
        .iter()
        .map(|(a, b, c)| format!("<{}, {}, {}>", a, b, c).to_owned())
        .collect::<Vec<String>>()
        .join(";");

    // println!(
    //     "line: {}, ({}) line1: {}, line2: {}, nums: {} + {} = {}",
    //     line_raw, rs, line1, line2, first, last, num
    // );
